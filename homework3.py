...
class robot:
    ...
    def move(self, motion, withNoise=True): # Do not change the name of this function
        alpha = motion[0]
        distance = motion[1]
        if (withNoise):
            distance += random.gauss(0, distance_noise)
        beta = (distance / self.length) * tan(alpha)
        x = self.x
        y = self.y
        theta = self.orientation
        newOrientation = theta + beta
        if (withNoise):
            newOrientation += random.gauss(0, steering_noise)
        if abs(beta) < 0.001:
            ...
            cx = x - r * sin(theta)
            cy = y + r * cos(theta)
            x = cx + r * sin(newOrientation)
            y = cy - r * cos(newOrientation)
        thetaN = newOrientation % (2 * pi)
        result = robot(self.length)
        result.set_noise(bearing_noise, steering_noise, distance_noise)
        result.set(x, y, thetaN)
    ...
    # sense:
    #
    def sense(self, withNoise = True): #do not change the name of this function
        Z = []
