...
def planLineal(road, lane_change_cost, init, goal): # Don't change the name of this function!
    #
    #
    # Insert Code Here
    #
    ...
    print 'init', init
    print 'goal', goal
    print 'lane change cost', lane_change_cost
    current = init
    print 'current', current, road[current[0]][current[1]]
    cost = 0
    neighbours = getNext(current, road, lane_change_cost, goal)
...
#        print 'next', next
        cost += next[0]
#        print 'current velocity', road[next[1]][next[2]]
        current = [next[1], next[2]]
        print 'current', current, road[current[0]][current[1]]
        neighbours = getNext(current, road, lane_change_cost, goal)
#        print 'neighbours', neighbours
        found = isGoal(neighbours, goal)
    cost += 1.0 / road[goal[0]][goal[1]]
#    if (current[0] != goal[0]):
            ...
            for y in range(len(road[0])):
                if goal[0] == x and goal[1] == y:
                    ...
                    for a in range(len(delta)):
                        ...
                        if x2 >= 0 and x2 < lanes and y2 >= 0 and y2 < len(road[0]) and road[x2][y2] != 0:
                            cost = 1.0 / road[x2][y2]
                            if x2 != x:
                                cost += lane_change_cost
                            v2 = value[x2][y2] + cost
...
def getNext(current, road, laneChangeCost, goal):
    result = []
    lane = current[0]
    currentX = current[1]
    nextX = currentX + 1
    ...
    for i in range(-1, 2):
        nextLane = lane + i
        difLaneToGoal = abs(goal[0] - nextLane)
        difStepsToGoal = abs(nextX - goal[1])
        if (nextLane >= 0 and nextLane < lanes and road[nextLane][nextX] != 0 and difStepsToGoal >= difLaneToGoal):
#            print 'road', road[nextLane][nextX]
            cost = 1.0 / road[nextLane][nextX]
#            print 'cost without lane change', cost
            if nextLane != lane:
                cost += laneChangeCost
#            print 'cost', cost
            result.append([cost, nextLane, nextX])
    return result

def isGoal(nodes, goal):
    found = False
    for node in nodes:
        if goal[0] == node[1] and goal[1] == node[2]:
            found = True
            break
    return found
...
def solution_check(test, epsilon = 0.00001):
    ...
    for i in range(len(test[0])):
        ...
        true_cost = test[4][i]
        print 'Expected cost', true_cost
        print 'Calculated cost', user_cost
